...
public class CreateMessageCommandHandler : IRequestHandler<CreateMessageCommand, Message>
{
    ...
    public async Task<Message> Handle(CreateMessageCommand command, CancellationToken cancellationToken)
    {
        ...
        var userMessage = new Message(command.ConversationId, command.Role, command.Content, null, _dateTime.UtcNow());
        var assistantMessage = new Message(command.ConversationId, Roles.Assistant,
            string.Empty, command.StreamId, _dateTime.UtcNow().AddMilliseconds(1));
        await _messageRepository.Save(new[] { userMessage, assistantMessage }, cancellationToken);
        ...
        assistantMessage.UpdateContent(firstChoice.Message.Content);

        // save both user message and assistant message
        await _messageRepository.Save(new[] { assistantMessage }, cancellationToken);
